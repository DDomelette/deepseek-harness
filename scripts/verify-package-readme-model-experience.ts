...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-settings-mcp': { kind: 'none', reason: 'Browser-side settings surface over the Host-owned MCP roster; the mounted dsh-mcp-client instances own every model-visible effect.' },
  'packages/client/ui-settings-models': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
  'packages/client/ui-settings-skills': { kind: 'none', reason: 'Browser-side settings surface; the Host skill-settings and skill consumers own every model-visible consequence of its writes.' },
  ...
  'packages/skill/skill-settings': { kind: 'indirect', reason: 'The registry override changes invocation policy; dsh-tool-skill and user-explicit invocation own the rendered catalog, tool behavior, and body injection.' },
