...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It binds every interface and serves `http://127.0.0.1:3080` locally, opening that canonical host URL only after the complete Loader tree settles. A non-empty inherited `SSH_CONNECTION` or `SSH_TTY` suppresses the browser handoff because the SSH client or editor owns the local forwarded address; the host URL is still printed. Because every interface is bound, it prints a plain-HTTP warning on stderr and the readiness line lists the token-free LAN URLs a phone can open; `--host 127.0.0.1` restricts the invocation to this machine, and `--allow-lan` is accepted for compatibility but no longer gates anything. The web profile layers [`@deepseek-ai/dsh-mob`](../../../packages/bundle/mob/README.md) as well: Settings → General → Connect phone renders a one-time pairing link as a QR code for a phone on the same network. Immediately before a local handoff it prints `dsh web: opening the default browser; pass --no-open to disable`; if the operating-system handoff fails, a diagnostic on stderr states the reason, leaves the server running, and names the URL for manual use. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
