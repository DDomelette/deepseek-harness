...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.sessionDeletion` | `core` | [`session-deletion`](../packages/session/session-deletion) | - | [`api-session-controller`](../packages/api/session-controller) | - | Coordinates durable leaves-first persistence and Workspace cleanup. |
| `ctx.sessionFlags` | `core` | [`session-flags`](../packages/session/session-flags) | - | [`session-pins`](../packages/session/session-pins) | - | Combines synchronous provider flag maps with explicit completeness. |
| `ctx.sessionPins` | `core` | [`session-pins`](../packages/session/session-pins) | - | - | - | Stores pin membership and per-view ordering independently of Workspace membership. |
