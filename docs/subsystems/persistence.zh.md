 ...
 */
delete( input: { readonly sessionId: SessionId; readonly recursive: boolean }, detach?: (ids: readonly SessionId[]) => Promise<void>, ): Promise<{ readonly deletedSessionIds: SessionId[] }>
```
...
```ts cordis-catalog
/**
 * Permanently remove a stored Session and its artifacts after all writers close.
 * Explicit deletion removes every committed generation; migration alone never does.
 * @param id - Session whose durable data is removed.
 * @returns completion after removal; missing Sessions reject.
