...
- 延长的前提是手机手里那张 cookie **尚未过期**：过期之后每一次请求（包括索引请求）都会被拒绝，刷新路径也就不可达，那台手机必须重新配对。窗口仍然有效时，手机加载一次页面即对齐到延长后的窗口；在此之前 `/api` 请求仍受旧载荷的到期时间约束。面板与文档的措辞都要带上这个前提，不假装延长对任何状态的手机都即时生效。
