...
Run: `pnpm vitest run packages/mcp/mcp-manager/tests/schema.spec.ts`
Expected: FAIL（模块不存在）
...
Run: `pnpm vitest run packages/mcp/mcp-manager/tests/register.spec.ts`
Expected: FAIL
...
README.md / README.zh.md / README.i18n.yaml：参照 `packages/mcp/mcp-client/README*` 的双语结构与 i18n.yaml 格式，内容覆盖：能力（settings 命名空间 + 热挂载 + 只读 declarative 投影）、配置表、与 mcp-client 的关系。

- [ ] **Step 9: 安装依赖并运行测试**

Run: `pnpm install && pnpm vitest run packages/mcp/mcp-manager`
Expected: PASS
...
Run: `pnpm vitest run packages/mcp/mcp-manager/tests/supervisor.spec.ts`
Expected: FAIL
...
Run: `pnpm vitest run packages/mcp/mcp-manager/tests/mount.spec.ts`
Expected: FAIL
...
Run: `pnpm vitest run packages/mcp/mcp-manager`
Expected: PASS
...
Run: `pnpm vitest run packages/mcp/mcp-manager/tests/gateway.spec.ts`
Expected: FAIL
...
Run: `pnpm install && pnpm run build:lib:host`（tsdown 的 typertPlugin 自动扫描 `TypertRemoteService` 子类并生成 artifacts）
Expected: `packages/mcp/mcp-manager/lib/typert.host.js` 与 `lib/typert.remote-client.js` 生成
...
Run: `pnpm vitest run packages/host/apiproxy/tests/api-proxy-config.spec.ts -t mcp-servers`
Expected: FAIL（`settings-not-exposed` 或 namespaces 不含）
...
Run: `pnpm install && pnpm vitest run packages/host/apiproxy/tests/api-proxy-config.spec.ts`
Expected: PASS

- [ ] **Step 5: 类型检查**

Run: `pnpm run build:lib:host && pnpm run typecheck:contracts-ready`
Expected: 编译通过
...
Run: `pnpm vitest run packages/client/ui-settings-mcp`
Expected: FAIL
...
Run: `pnpm vitest run packages/client/ui-settings-mcp --coverage`
Expected: PASS，新包源文件 100% 覆盖

- [ ] **Step 6: bundle 与类型检查**

Run: `pnpm --filter @deepseek-ai/dsh-client-ui-settings-mcp bundle && pnpm run typecheck:contracts-ready`
Expected: 编译通过
