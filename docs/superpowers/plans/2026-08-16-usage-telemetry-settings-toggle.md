...
export class UsageTelemetryCardController {
  ...
  readonly store: SnapshotStore<UsageTelemetryCardState> = createSnapshotStore(this.projection())
...
it('renders the card title and stages the opposite switch value', () => {
  const edit = vi.fn()
  render(<UsageTelemetryCard
    useUsageTelemetryCard={selector => selector(state)}
    edit={edit}
    resetField={() => {}}
    save={() => {}}
    discard={() => {}}
