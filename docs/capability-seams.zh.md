...
flowchart LR
  ...
  svc_sessionDeletion["ctx.sessionDeletion<br/>Permanent Session deletion"]
  ...
  svc_sessionPins["ctx.sessionPins<br/>Pinned Session membership"]
  ...
  svc_sessionDeletion --> pkg_api_session_controller
  svc_sessionFlags --> pkg_session_pins
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.sessionDeletion` | `core` | [`session-deletion`](../packages/session/session-deletion) | - | [`api-session-controller`](../packages/api/session-controller) | - | 协调从叶节点开始的持久化与工作区清理。 |
| `ctx.sessionFlags` | `core` | [`session-flags`](../packages/session/session-flags) | - | [`session-pins`](../packages/session/session-pins) | - | 合并同步提供方标记映射并明确报告完整性。 |
| `ctx.sessionPins` | `core` | [`session-pins`](../packages/session/session-pins) | - | - | - | 独立于工作区成员关系存储置顶成员及各视图顺序。 |
