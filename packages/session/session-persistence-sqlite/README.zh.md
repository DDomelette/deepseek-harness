...
`SqliteSessionPersistence.delete(id)` 在该会话的按 id 串行链上委托给共享 `PersistenceCoordinator`。协调器把删除串行排在会话退役流程之后，经存储层移除任何已实体化产物，把已跟踪但未实体化的创建意图视为可取消状态，清除协调器状态与准备缓存，并且仅在操作成功后发出 `session-persistence/deleted`。既未被跟踪、也不存在于存储中的 id 会以 `SessionPersistenceNotFoundError` 拒绝。

`SqliteStore.deleteStored(id, signal?)` 会在打开数据库前、以及开始同步变更前观察可选取消信号。随后它持有 `BEGIN IMMEDIATE`，重新校验可变 schema 与 application identity，执行由本包拥有的 `delete-session` SQL 资源并提交；任何失败都会回滚。删除 `sessions` 行时，已启用的 `ON DELETE CASCADE` 外键会原子移除其 `events` 行。存在一个会话行时，该方法返回 `true`；不存在时返回 `false`。同步 SQLite 事务开始后，取消信号无法中断它。

本包在 [`src/sql.ts`](src/sql.ts) 的封闭列表中以 `delete-session` 条目拥有该语句，实际内容位于 [`resources/sql/delete-session.sql`](resources/sql/delete-session.sql)。运行时 TypeScript 仅以 SQLite 参数提供会话 id，不会组装删除查询文本。
...
- **没有后台历史压缩**——普通追加只做插入；显式整会话删除与撕裂尾部修复是仅有的已提交行移除路径。
