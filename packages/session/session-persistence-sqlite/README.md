...
`SqliteSessionPersistence.delete(id)` delegates to the shared `PersistenceCoordinator` on that session's per-id chain. The coordinator serializes deletion after lifecycle retirement, removes any materialized artifact through the store, treats a tracked but unmaterialized create intent as cancellable state, clears coordinator and preparation state, and emits `session-persistence/deleted` only after the operation succeeds. An id that is neither tracked nor stored rejects with `SessionPersistenceNotFoundError`.

`SqliteStore.deleteStored(id, signal?)` observes optional cancellation before opening the database and again before starting the synchronous mutation. It then holds `BEGIN IMMEDIATE`, revalidates the mutable schema and application identity, executes the package-owned `delete-session` SQL resource, and commits; any failure rolls back. Deleting the `sessions` row atomically removes its `events` rows through the enabled `ON DELETE CASCADE` foreign key. The method returns `true` when one session row existed and `false` when none did. Once the synchronous SQLite transaction starts, an abort signal cannot interrupt it.

The package owns the statement as the closed `delete-session` entry in [`src/sql.ts`](src/sql.ts), backed by [`resources/sql/delete-session.sql`](resources/sql/delete-session.sql). Runtime TypeScript supplies the session id as a SQLite parameter; it does not assemble deletion SQL.
...
- **No background historical compaction** — normal appends are insert-only; explicit whole-session deletion and torn-tail repair are the only paths that remove committed rows.
