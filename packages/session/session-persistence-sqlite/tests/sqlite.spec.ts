...
describe('SessionPersistenceSqlite physical packing', () => {
  ...
  it('deleteStored removes the session artifact and its event rows', async () => {
    ...
    const header = meta('delete-stored')
    await store.appendBatch(header, chunkLog(3), false)

    await expect(store.deleteStored(header.id)).resolves.toBe(true)
    await expect(store.loadStored(header.id)).resolves.toBeUndefined()
