...
`delete(id)` 是显式永久删除：拒绝已占用的写入者，持有同一操作系统写锁，按从旧到新的顺序删除所有会话代际及附件，最后删除当前代际。在 POSIX 上，锁文件及空目录保留以维持锁身份。只读迁移准备保留源代际且不获取写锁；发布后继代际和显式删除需要获取写锁。
