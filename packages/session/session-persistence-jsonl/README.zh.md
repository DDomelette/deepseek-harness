...
`delete(id)` 是显式永久删除：拒绝已占用的写入者，持有同一操作系统写锁，按从旧到新的顺序删除所有会话代际及附件，最后删除当前代际。锁文件及空目录保留以维持锁身份。历史日志的读取迁移也获取此锁；普通读取和迁移保留源代际。Windows 使用 Koffi，POSIX 动态加载可选原生依赖 `fs-ext`；POSIX 缺失该依赖时操作失败。
