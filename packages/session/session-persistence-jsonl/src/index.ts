...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    }, 'session-persistence-jsonl.deletions')
  }
  ...
  async open(id: SessionId, access: SessionAccess, options?: SessionPersistenceOpenOptions): Promise<SessionHandle> {
    ...
    try {
      ...
      const stored = await this.requireStoredLog(id, options?.signal, true)
  ...
  /**
   * Remove every generation and attachment while holding the Session's writer lock.
   * The lock directory remains so another POSIX writer cannot lock a replacement inode.
   * @param id - stored Session to remove after its writer closes.
   * @returns completion of artifact removal and lock release.
   */
  delete(id: SessionId): Promise<void> {
    if (this.deletionClosing) return Promise.reject(new Error('session persistence is closing'))
    const operation = this.deleteArtifacts(id)
    this.deletions.add(operation)
    void operation.then(() => { this.deletions.delete(operation) }, () => { this.deletions.delete(operation) })
    return operation
  }

  private async deleteArtifacts(id: SessionId): Promise<void> {
    await this.ensureRootEncoding()
    this.tracker.claimWrite(id)
    let lease: SessionWriteLease | undefined
    try {
      ...
      const generations = entries.flatMap((name) => {
        const version = parseGenerationLogFilename(name, this.compression)
        return version === undefined ? [] : [{ name, version }]
      }).sort((left, right) => left.version - right.version)
      if (generations.length === 0) throw new SessionPersistenceNotFoundError(id)
      // Remove predecessors first: a concurrent header read never falls back to an older generation.
      for (const entry of entries) {
        if (entry === LEASE_FILENAME || generations.some(generation => generation.name === entry)) continue
  ...
  private async requireStoredLog(id: SessionId, signal?: AbortSignal, leaseHeld = false): Promise<StoredLog> {
    ...
    const current = await this.ensureCurrentLog(id, signal, selected, leaseHeld)
  ...
  private async ensureCurrentLog(
    ...
    if (selected === undefined) return undefined
    if (selected.sourceVersion < SESSION_FORMAT_VERSION && !leaseHeld) {
      const lease = await this.acquireLease(id, undefined, dirname(selected.sourcePath))
      try {
        const current = await this.findLog(id, signal)
        if (current === undefined) return undefined
        return await this.ensureCurrentLog(id, signal, current, true)
      } finally {
        await lease.release()
      }
    }
