...
`delete(id)` permanently removes a Session by explicit request: it refuses existing writers, holds the same OS write lease, and removes attachments and generations from oldest to newest, deleting the current generation last. The lease file and empty directory retain their lock identity. Read-triggered historical migration also acquires this lease; ordinary reads and migration preserve source generations. Windows uses Koffi; POSIX dynamically loads the optional native `fs-ext` dependency and fails if it is unavailable.
