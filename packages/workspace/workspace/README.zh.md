...
当会话不应再出现在分组中时隐藏它：它会从可见列表中消失，但其会话、历史与在项目中的位置都保持不变。恢复会话会让它重新出现在保留的位置。项目不再需要时移除它：它离开列表，而其文件夹、文件与会话历史绝不受影响——这些会话变成 Ungrouped。之后再次添加同一目录会从空项目开始，不会带回旧会话。
...
注册表打开 `workspace` 领域（版本 2）：一张以 `WorkspaceId` 为键的 `workspaces` 表，加上一个持有 `workspaceIds`（权威显示顺序）、`archivedSessionIds`、`archivedSessionAts` 与可选 `pendingMutation` 标记的全局状态。恢复或遗忘会话会同时移除其归档成员资格与时间戳。在 `archivedSessionIds` 存在之前写入的记录会通过 schema 默认值解析为空集合。
