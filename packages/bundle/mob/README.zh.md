---
description: "dsh 面向 web profile 的局域网手机加入层：设置入口通过一次性短码完成配对，并管理已批准的设备。"
kind: "package-bundle"
---
...
启动 `dsh web`，再打开 设置 → 通用设置 → 连接手机，即可为同一网络中的手机配对：面板申请一次性短码，把 `/pair?c=<code>` 链接渲染成二维码，并对每个请求的手机做出决定。该层在 `web` 表层之上加入这个加入入口与应答它的 `mob` Remote 命名空间；局域网绑定及其警告、进程令牌交换（仅回环）与签名 cookie 认证仍是 `dsh web` 的行为。服务仍是明文 HTTP，因此只在可信网络上使用。
...
`web` profile 组合了本组合包，因此手机接入就是 `dsh web` 的普通能力：就绪行在回环 URL 旁打印局域网 URL，而那条局域网行**不带**进程令牌——令牌是电脑自己的凭据，只在回环上交换。因此手机通过配对加入，拿到属于它自己的设备 cookie，而不是电脑会话的副本。`dsh web` 绑定所有网卡，因此局域网一侧开箱可达；`--host 127.0.0.1` 把该次调用限制在本机，此时配对入口会提示内网访问未开启。

### 从桌面会话交接

浏览器半层在 设置 → 通用设置 中加入「连接手机」行，它是唯一的加入入口——终端不打印任何内容。该行打开配对面板：「生成配对码」通过 `POST /pair/session` 向 Host 申请短码，以本机局域网 origin 拼出不带进程令牌的 `/pair?c=<code>` 链接，并把该链接渲染成二维码，旁边显示短码与剩余秒数。随后面板列出待确认的请求——每条都带有从手机 agent 推导出的设备名称，可在「允许」或「拒绝」之前修改——以及已配对的设备，显示添加时间、最近一次通过认证的时间，以及一个「吊销」按钮，让该设备的下一次请求失效。不是电脑本机的页面看不到这些操作；仅绑定回环的部署会明确说明：加入 URL 以 `mob/loopback-only` 失败（去掉 `--host 127.0.0.1`），或当 all-interfaces 绑定推导不出可达地址时以 `mob/no-lan-address` 失败（需要修复本机网络）。所有操作都走手机端使用的同一批 `/pair*` 路由，因此守护「决定」的「回环 + 会话」规则只有一个执行点。每条已配对设备还会显示寿命：`{days} 天（剩余 {remaining} 天）`、窗口已过的「已过期」，或旧条目在操作者设定天数之前的 `—`（旧条目仍按 cookie 载荷里的到期时间运行）。该行的 `1`/`7`/`30`/`90` 天档位与任意天数输入会重新开始该设备的倒计时；缩短在手机下一次请求即生效，延长则在手机下一次打开页面时生效。列表上方的一行文案提示：不再使用的设备立即吊销，在不受信任的网络上用过之后也建议吊销。
...
局域网 origin 来自 `webRuntime` 服务——即 `dsh-web-app` 喂给 `/api` 信任栅栏的同一份 `resolveLanTrust` 快照——因此以它为基拼出的配对链接总能通过栅栏。第一个非内部 IPv4 字面量与绑定端口一起构成该 origin；它不带进程令牌，空快照（仅回环）会让该调用失败，而不是给出一个 origin。
