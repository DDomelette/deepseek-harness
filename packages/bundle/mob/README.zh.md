...
启动 `dsh web --host 0.0.0.0 --allow-lan`，再打开 设置 → 通用设置 → 连接手机，即可为同一网络中的手机配对：面板申请一次性短码，把 `/pair?c=<code>` 链接渲染成二维码，并对每个请求的手机做出决定。该层在 `web` 表层之上加入这个加入入口与应答它的 `mob` Remote 命名空间；局域网绑定及其警告、进程令牌交换（仅回环）与签名 cookie 认证仍是 `dsh web` 的行为。服务仍是明文 HTTP，因此只在可信网络上使用。
...
`web` profile 组合了本组合包，因此手机接入就是 `dsh web` 的普通能力：就绪行在回环 URL 旁打印局域网 URL，而那条局域网行**不带**进程令牌——令牌是电脑自己的凭据，只在回环上交换。因此手机通过配对加入，拿到属于它自己的设备 cookie，而不是电脑会话的副本。`--allow-lan` 是在可信网络提供服务所需的显式确认；没有它，全接口绑定仍是用法错误。
