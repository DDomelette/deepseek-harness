...
- **LAN serving is plain HTTP, and it is the default** — every interface is bound unless `--host 127.0.0.1` is passed, and traffic stays unencrypted; a stolen device cookie grants full control, so serve only on a trusted network.
