...
- **按设备的防火墙规则跟随手机地址**——上面的配方只放行一个 IP 字面量，DHCP 租约变化后需要重新添加规则；不再使用的设备在你吊销它并删除规则之前，仍然可以访问该端口。租约转给另一台设备后，那台设备会获得同样的放行，因此要么为手机保留静态租约，要么去掉 `remoteip` 让规则只放行该端口，并在用完后吊销该设备。
