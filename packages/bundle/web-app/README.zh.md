...
运行 `dsh --profile web`，打开提供聊天、模型与设置管理以及会话历史的交互式浏览器 GUI。它使用与其他 dsh 表层相同的模型访问、工具与安全默认值。启动时绑定所有网卡，打印带回环认证信息的 URL 与局域网 URL，通常还会在默认浏览器中打开回环 URL；SSH 会话和 `--no-open` 会保留该 URL，供你手动打开。你可以更改端口、允许额外主机，并用 `--host 127.0.0.1` 只服务本机。需要在浏览器中交互式工作时选择本包；一次性的命令行任务应使用 `dsh-headless`。
...
- **LAN 服务是明文 HTTP，且是默认行为**——除非传 `--host 127.0.0.1`，否则绑定所有网卡，流量不加密；被盗的设备 cookie 会授予完全控制权，因此只在信任的网络上提供服务。
