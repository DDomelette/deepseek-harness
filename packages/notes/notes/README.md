...
- **Nothing collects a material yet** — the panel lists, edits, submits, and answers materials, but no surface in a transcript collects one, so a deployment reaches `notes/materialAddText` through the Remote namespace alone until the selection bubble lands.
