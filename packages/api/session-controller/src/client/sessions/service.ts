...
export class ClientSessions implements ISessions {
   ...
   */
  /**
   * Permanently delete the addressed Session tree and refresh Client membership.
   * @param sessionId - root Session identity.
   * @returns deleted identities.
   */
  async deleteSession(sessionId: SessionId): Promise<readonly SessionId[]> {
    ...
    if (current !== undefined && result.value.deletedSessionIds.includes(current)) this.clear()
    for (const id of result.value.deletedSessionIds) this.manager.handleSessionRemoved(id)
    await this.refresh()
