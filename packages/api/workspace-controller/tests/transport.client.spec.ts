...
class CommandWorkspaceRemote implements WorkspaceRemote {
  ...
  readonly archiveSession = vi.fn<WorkspaceRemote['archiveSession']>(request => Promise.resolve(remoteOk({ archivedSessionAts: {},
  ...
  it('delivers one baseline followed by increments', async () => {
    ...
    const remote = new ScriptedWorkspaceRemote([{
      frames: [
        ...
        { archivedSessionAts: {}, type: 'archived', archivedSessionIds: ['session-one' as never] },
      ],
      hold: true,
    }])
...
describe('WorkspaceController', () => {
  it('publishes the model source and exposes successful Workspace commands', async () => {
    ...
    model.replaceBaseline({ archivedSessionAts: {}, items: [workspace('one')], archivedSessionIds: [] })
