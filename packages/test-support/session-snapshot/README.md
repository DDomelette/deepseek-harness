...
`materializeProfilePatch` links the replay provider from this package's declared `dsh-llm-replay` dependency into the temporary profile. Other bare plugins resolve from the authored patch's installation or the dsh installation; relative plugin paths become absolute before the subprocess starts.
