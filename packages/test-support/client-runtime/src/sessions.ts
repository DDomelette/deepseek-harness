...
export class TestSessions implements ISessions {
  ...
  readonly calls: {
    method: 'create' | 'open' | 'openSubagent' | 'setSubagentCatalogOpen' | 'refreshSubagents'
      | 'clear' | 'refresh' | 'search' | 'fork' | 'deleteSession'
    args: unknown[]
  }[] = []

  /**
   * Record deletion and remove its addressed Session from the fixture list.
  ...
  async deleteSession(sessionId: SessionId): Promise<readonly SessionId[]> {
    ...
    await this.stabilize(() => { this.list.update((draft) => {
      draft.ids = draft.ids.filter(id => id !== sessionId)
      draft.byId = Object.fromEntries(Object.entries(draft.byId).filter(([id]) => id !== sessionId))
