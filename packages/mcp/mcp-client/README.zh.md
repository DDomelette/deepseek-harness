...
以上声明式条目仍是唯一的组合期接入方式；本包自身没有设置界面。Web 设置面板通过 [`@deepseek-ai/dsh-mcp-manager`](../mcp-manager/README.zh.md) 管理另一份名单，该管理器为每个启用的 `mcp-servers` 条目热挂载一个本包实例。两份名单并存：管理器拒绝与声明式行重名的设置条目，本包自身按 root 的 `serverName` 保留是加载期的兜底。
