...
skill 可以是单层目录 bundle（`<name>/SKILL.md`），也可以是平铺 Markdown 文件（`<name>.md`）。刻意不支持发现嵌套的 `**/SKILL.md`。Frontmatter 使用 `yaml` 包解析为开放的 YAML 对象；该提供方解析必填的 `name` 和 `description`，以及可选的 `whenToUse`、`group`、`metadata`、`disable-model-invocation` 和 `user-invocable`。名称必须使用 kebab-case。

可选的 `group` 字符串是展示标签：聚合界面（技能设置面板）据此分组，注册表将其视为不透明元数据。类型错误的 `group` 会像类型错误的 `whenToUse` 一样被省略，因为分组不授予任何调用权限。
