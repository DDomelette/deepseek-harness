...
`SkillInvocationOverride` supplies an optional invocation policy for one summary. Overrides register with a disposer and can disable model and user invocation without hiding invocation-neutral metadata from settings. A display `group` is optional and independent of invocation eligibility.
