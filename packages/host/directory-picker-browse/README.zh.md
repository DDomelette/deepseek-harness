...
`list(path?)` 返回一个目录层级：按名称排序的子目录及其绝对路径、`hidden` 标志（POSIX 上为点前缀）、`home` 锚点，以及 `crumbs`——从根到目标的祖先链，其中每个 crumb 都是跳转目标，根以完整路径标注。不带路径时列举宿主账户的家目录。单次调用至多返回 `maxEntries` 行（配置项，默认 1,000——GitHub 网页端对目录列举采用的同一上限），被截断的层级会报告 `truncated: true`，供客户端提示层级不完整。指向目录的符号链接会被跟随；断链与循环链接被跳过。在 Windows 上，一个层级还会带上 `drives`：按盘符顺序排列的已挂载卷，每次调用都重新探测，因为该祖先链止于被列举路径自己的盘符，而后挂载的卷也必须出现。其他平台不带该字段。

在 WSL Host 上，路径输入也接受 `D:\projects` 等带盘符的完整 Windows 路径和以反斜杠表示的 UNC 路径。后端先通过 `wslpath -u` 解析实际挂载位置，再列举目录或创建子目录。返回的路径和面包屑使用 Linux 写法，因此打开所选项会注册刚才列举的同一目录。其他 POSIX 宿主不转换 Windows 路径；转换失败仍使用常规目录错误码。
