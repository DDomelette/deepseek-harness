...
- 每行标题下方显示归档日期；在该字段出现之前归档的会话显示“归档时间未知”。
- 详情按钮打开固定字段对话框：分组、目录、Agent 预设、归档时间、最后活动、状态、子代理对话数与会话 ID。
- 恢复会取消归档、打开会话并关闭设置页。
- 删除请求递归删除，并且总是先打开确认对话框。删除尚未完成时，确认和取消按钮禁用，关闭操作也会保留对话框。
- 运行中的行保留恢复按钮，禁用删除按钮。
...
本包没有模型可见界面。插件只渲染已归档对话 `settings.section` 条目，不注册工具、提示词或会话事件。
