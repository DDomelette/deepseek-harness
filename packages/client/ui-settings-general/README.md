...
Users reach the shell through the sidebar's bottom Settings control; feature plugins contribute their pages and onboarding steps through the slot ledgers this shell projects. In both the expanded sidebar and collapsed rail, the control exposes the localized Settings label as its accessible name. A pale-yellow **Disconnected** action beside Settings indicates browser offline suspension. Automatic recovery shows **Reconnecting** with one to three dots advancing every 500ms. Hover or keyboard focus changes either yellow label to **Reconnect now** without changing its background; press feedback stays within the warning palette, and selecting it starts retry 1 immediately. Recovery changes the region to pale-green **Connected** for two seconds before it disappears. The icon, left-aligned text origin, height, and width remain fixed across every visible state. Initial startup and uninterrupted healthy operation remain silent. The shell renders the modal panel, the navigation built from `settings.section` entries, and exactly one mounted onboarding step at a time. Below the handset breakpoint the panel fills the viewport and shows one pane at a time: the section list first, then the chosen section under a back control that returns to the list, with the section name as the header text and the header's open-document action collapsed to an icon-only button that keeps its accessible name; Escape leaves the section before it leaves the dialog, and under `box-sizing: border-box` the sheet pads itself with `env(safe-area-inset-*)` so its content stays inside the viewport safe area. Wider viewports keep the nav rail and the content column side by side.
