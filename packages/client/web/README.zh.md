...
| [`src/compat.ts`](src/compat.ts) | 浏览器底座：外壳的 bundle 期望由更新引擎提供的 API |
...
- **浏览器底座逐条列举 API**——若某个 bundle 调用了比外壳所服务引擎更新的其他 API（例如 `Object.groupBy`），在这些引擎上会失败，直到其定义加入 `src/compat.ts`；目前没有任何机制发现这种遗漏。
