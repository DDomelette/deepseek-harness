...
describe('ui-workspace apply', () => {
  ...
  it('declares the two directory-flow holes and reports their occupancy per surface', async () => {
    ...
    expect(b.slots.spec('sidebar.workspaces.pinned')).toMatchObject({ kind: 'single' })
    expect(b.slots.spec('sidebar.workspaces.sessionActions')).toMatchObject({ kind: 'list' })
    expect(b.slots.spec('sidebar.workspaces.searchResultExtra')).toMatchObject({ kind: 'list' })
