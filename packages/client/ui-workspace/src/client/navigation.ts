...
/** Feature-owned presentation flags; they do not change Host Workspace membership. */
export type WorkspaceSessionFlags = Readonly<Record<SessionId, { readonly pinned?: boolean }>>

/** Workspace archive and directory operations consumed by Client UI domains. */
export interface UiWorkspace {
  /** Merged feature presentation flags for the Workspace browser. */
  readonly sessionFlags: HostObservable<WorkspaceSessionFlags>
  /**
   * Contribute live presentation flags while a feature is mounted.
   * @param source - feature-owned flags and invalidations.
   * @returns disposer that withdraws the contribution.
   */
  registerSessionFlags(source: HostObservable<WorkspaceSessionFlags>): () => void
  /**
...
class UiWorkspaceService extends Service implements UiWorkspace {
  readonly sessionFlags = createSnapshotStore<WorkspaceSessionFlags>({})
  private readonly flagSources = new Set<HostObservable<WorkspaceSessionFlags>>()

  registerSessionFlags(source: HostObservable<WorkspaceSessionFlags>): () => void {
    const refresh = (): void => {
      const flags: Record<SessionId, { pinned?: boolean }> = {}
      for (const provider of this.flagSources) {
        for (const [id, value] of Object.entries(provider.getSnapshot())) {
          flags[id as SessionId] = { ...flags[id as SessionId], ...value }
        }
      }
      this.sessionFlags.set(flags)
    }
    this.flagSources.add(source)
    const off = source.subscribe(refresh)
    refresh()
    return () => { off(); this.flagSources.delete(source); refresh() }
  }
