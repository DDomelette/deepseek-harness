...
> & {
  ...
  renderSessionActions: (sessionId: SessionId) => ReactNode
...
function SessionTree({
  ...
  revealSessionId, onSessionRevealed, renderSessionActions,
}: SessionTreeProps) {
              ...
              // browser-local account; real Workspaces may also write Host order.
                ...
                return (
                  <SessionNodeItem
                    key={node.id}
                    node={node}
                    extraActions={renderSessionActions(node.id)}
                    currentId={current}
                    now={now}
                    onOpen={open}
...
function FlatList({
  ...
  revealSessionId, onSessionRevealed, renderSessionActions, t,
}: Pick<
  SessionTreeProps,
  | 'useSessions'
...
function SearchResults({
  ...
  resultLimit: number
  renderSearchResultExtra: (sessionId: SessionId) => ReactNode
}) {
  const panelActive = usePanelInfo(info => info.activePanelId !== null)
  const list = useSessions(s => s)
  const pendingInteractions = useSessionPendingInteraction(s => s)
        ...
        <div className={css.searchTree} role="tree" aria-label={t('search.results.aria')}>
          {results.items.map(result => (
            <SearchResultItem
              key={result.id}
              result={result}
              extra={renderSearchResultExtra(result.id)}
              currentId={panelActive ? undefined : list.current}
              onOpen={open}
              t={t}
            />
...
export function WorkspaceBrowser({
  ...
  const sessionFlags = useWorkspaceSessionFlags(state => state)
  const hiddenSessionIds = useMemo(() => [...new Set([
    ...archivedSessionIds,
    ...Object.entries(sessionFlags).filter(([, flags]) => flags.pinned).map(([id]) => id as SessionId),
  ])], [archivedSessionIds, sessionFlags])
  ...
  const openSearchResult = (sessionId: SessionId): void => {
    setRevealSessionId(sessionFlags[sessionId]?.pinned ? undefined : sessionId)
    setQuery('')
  ...
  return (
    <div className={clsx(css.root, !wide && css.rail)}>
      ...
      {/* Always-mounted seat keeps the region's flex slot while the list
          itself is wide-only. */}
      {normalizedQuery === '' && renderSlot('sidebar.workspaces.pinned', { wide, view: groupBy === 'flat' ? 'flat' : 'grouped' })}
        ...
        {wide && (normalizedQuery !== ''
          ? (
            <SearchResults
              ...
              query={normalizedQuery}
              remote={remoteSearch}
              renderSearchResultExtra={sessionId => renderSlot('sidebar.workspaces.searchResultExtra', { sessionId })}
          ...
          : groupBy === 'flat'
            ? (
              <FlatList
                ...
                archivedSessionIds={hiddenSessionIds}
                renderSessionActions={sessionId => renderSlot('sidebar.workspaces.sessionActions', { sessionId })}
                orderBy={orderBy}
