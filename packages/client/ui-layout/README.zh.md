...
窗口宽度低于 768px（`SIDEBAR_OVERLAY` 断点）时，展开的侧边栏脱离栏网格，以抽屉形式浮于中栏之上：框架保留 56px 控制栏作为抽屉的触发入口，一层 aria-hidden 遮罩盖住中栏，点击遮罩或按 Escape 关闭抽屉。打开或关闭抽屉时，中栏和右栏的宽度保持不变。侧边栏拖拽手柄在抽屉态下不渲染；上层界面已消费的 Escape 会让抽屉保持打开，因此一次按键只关闭最上层的界面。沿任一方向跨过 768px 或 1024px 断点都会丢弃窄屏展开覆盖，因此每个区间都以收起状态打开。
