...
Below the 768px `SIDEBAR_OVERLAY` breakpoint an expanded sidebar leaves the column grid and floats over the center as a drawer: the frame keeps the 56px rail as the drawer's trigger, an aria-hidden scrim covers the center, and a scrim tap or the Escape key closes the drawer. Opening or closing the drawer preserves the center and right column widths. The sidebar drag handle does not render beneath the drawer, and an Escape a higher surface already consumed leaves the drawer open, so one press dismisses only the topmost surface. Crossing the 768px or 1024px breakpoint in either direction drops the narrow-expansion override, so each band opens collapsed.
