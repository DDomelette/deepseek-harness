...
Requests without a TCP peer cannot establish local-operator access and do not initialize native socket-address rules.

Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. Each process mints a random launch token. `dsh-web-app` prints and opens the ordinary root URL with `?token=...`; `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /` with a loopback authority and TCP peer, writes an authority-bound signed cookie, and redirects to clean `/`. Its verdict tells the index owner what to serve: `serve` for an accepted session, `answered` when Connection wrote the response itself, and `auth-required` for a refusal on any other authority the Host/Origin fence accepts, where the shell is served as a 401 carrying the `__DSH_AUTH_REQUIRED__` boot fact — a phone holds no launch token, so pairing is its only way in. An authority the fence rejects never receives the shell; it gets the same minimal 401 text as loopback. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. The HTTP carrier accepts no query token outside the root exchange and no Authorization-header token.

The cookie signing secret is the owner-scoped `client-connection/browser-session` grant record in `ctx.credentials`. The local provider persists it in `$DSH_HOME/.credentials.yaml`; `BrowserAuth` loads or creates the record during Connection activation and retains the secret in memory, so request authentication is synchronous. Deleting or replacing the record takes effect on the next Connection activation. Cookies carry an absolute issue/expiry interval, defaulting to 30 days through `cookieMaxAgeDays`, and bind the normalized hostname plus port in both their deterministic name and signed payload. They are host-only, `Path=/`, `HttpOnly`, and `SameSite=Strict`; they deliberately omit `Secure` because the shipped server uses loopback HTTP. A launch-token cookie counts only with both a loopback authority and TCP peer — the process token is the computer's own credential, it is never exchanged on a LAN authority, and the LAN URL `dsh web` prints carries no token — so every other client presents the second cookie form instead: a `deviceId` issued for a paired device, accepted only while the `client-connection/paired-devices` grant record, which `BrowserAuth` also loads during activation, still lists that device, whose window the registry decides instead: a newly registered device starts at `deviceLifetimeDays` (default 30, legal 1–365) and an entry that predates the lifetime fields keeps running on the expiry its payload carries. A device cookie is accepted while the current time is before both that device's `expiresAt` and the expiry in its own payload, so a shortened window applies to the next request while a longer one reaches the phone on its next index request, which re-issues an aligned cookie, and only while its current cookie is still valid: a cookie past its payload expiry is refused on every request, index included, so that phone must pair again; `/api` requests never renew anything.

`src/devices.ts` owns that registry. `registerDevice` mints a 128-bit opaque id — branded as a `PairedDeviceId`, so revoke, touch, and cookie issuance cannot be handed a label or a pairing code — and stores the label the operator approved, `listDevices` reports the stored order, `revokeDevice` removes one entry so that device's cookie stops authenticating, and `touchDevice` advances the stored last-seen time at most once an hour so ordinary requests do not rewrite the credential file. Every write goes through `CredentialProvider.modifyRecord`, and a record this build cannot interpret fails the operation instead of being overwritten. The running cookie check also follows changes it did not make: `apply` re-reads the record on every `credentials/record-updated` for `client-connection/paired-devices`, so an operator editing the file, or a second process revoking a device, takes effect on the next request; a failed re-read is reported and keeps the previous set. `setDeviceLifetime` writes one device's `lifetimeDays` together with the `expiresAt` it implies, so setting a window restarts that device's countdown.
...
`ctx.connection` is also where a Host route asks the same two questions and mints device access: `requestRejection(request)` applies the fence and the browser session to a route outside `/api`, `isLocalOperatorRequest(request)` requires a loopback authority, an actual loopback TCP peer, and a valid launch-token cookie; forwarding headers and device cookies never grant operator access, and `connection.devices` owns the paired-device registry — `list`, `register`, `revoke`, `setLifetime(deviceId, days)`, `touch`, and `issueCookie(request, deviceId)`. A registry mutation refreshes the cookie check of the running Connection, so an approval or a revocation applies to the next request without a restart.
