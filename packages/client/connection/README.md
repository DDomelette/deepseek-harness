...
`src/devices.ts` owns that registry. `registerDevice` mints a 128-bit opaque id and stores the label the operator approved, `listDevices` reports the stored order, `revokeDevice` removes one entry so that device's cookie stops authenticating, and `touchDevice` advances the stored last-seen time at most once an hour so ordinary requests do not rewrite the credential file. Every write goes through `CredentialProvider.modifyRecord`, and a record this build cannot interpret fails the operation instead of being overwritten.
