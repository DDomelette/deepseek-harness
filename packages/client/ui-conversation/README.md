...
The content width axis reads the center column's live width rather than the viewport, so collapsing the sidebar widens the content without resizing the window: the shell publishes the column width as `--dsh-conversation-column-width`, and the content width resolves to `clamp(min(680px, column), column × 64%, 920px)` — below 680px the floor degrades to the column itself, so a handset column never overflows, and 920px caps line length. Before the first measurement, a 680px cap and `width: 100%` keep content within its parent and give mounted views nonzero width for scroll restoration. The edge width handles publish a pixel preference (`--dsh-chat-user-width`) that replaces the adaptive term wholesale, and they render only at widths of at least 768px with a fine pointer. The sticky composer seat adds `env(safe-area-inset-bottom)` padding to clear a handset's home-indicator band, and on coarse pointers the input bar buttons carry a 44px minimum touch target.
