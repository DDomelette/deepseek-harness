...
分组保存在浏览器本地。用户可创建分组、编辑成员及删除分组，全局与预设区块继续显示各自的启用和条件状态。成员键包含作用域，避免不同预设中的同名条目混淆；旧裸 ID 仅匹配全局条目。成员变更立即保存。删除当前选中的分组会关闭其成员对话框。长标题滚动显示，并尊重减少动态效果设置；在粗指针设备上，工具栏的分组选择控件带 44px 的最小触控目标。
