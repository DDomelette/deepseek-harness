...
面板隐藏时，会话 header 角落席位里的按钮（`conversation.session.header.corner`，在工具组右缘之外，与 Session 日志控件齐平）是回去的路。它的图形是左侧 sidebar 折叠图标的镜像。它与面板共用一个存储（slot 运行时允许两个同作用域席位共用一个 handle）；面板显示时它什么也不渲染。该角落是 list 席位，因此一次装配可以在它旁边注册别的控件——随包组合就把笔记控件放在那里——席位只占其占用者所需的宽度，而不是随之收起。于是折叠的 Sidebar 不花会话区任何代价：没有轨条、没有宽度，转录的滚动条留在列的边缘。没有会话就没有按钮也没有面板。
