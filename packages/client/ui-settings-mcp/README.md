...
The gear opens an inline editor prefilled from the redacted entry. Non-secret fields, including the resolved reconnect policy, show their current values; env/headers always start blank with the keep-unchanged hint, and only the fields the user changed enter one atomic group of deep path ops — a blank secret field never rewrites the stored secret. A refused or failed transaction keeps the editor open. Deleting asks for inline confirmation, clears the whole entry atomically, and closes only after Host acceptance.
