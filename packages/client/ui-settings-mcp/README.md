...
Manage MCP servers in the Plugins section of Web Settings. Inspect connection status and tool availability, search the roster, and add, configure, enable, disable, or delete a server. Secret values are write-only; the Host applies accepted settings changes.
