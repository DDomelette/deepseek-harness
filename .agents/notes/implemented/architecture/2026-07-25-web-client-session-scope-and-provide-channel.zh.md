# Agent Note: Web client Agent-scope 对等模型与供数通道（`runtime/src/client/agent-scope.ts` / blank 复用 / provide）
...
`runtime/src/client/agent-scope.ts` 与 host `dsh-scope` 机制层一致（fiber + tag + filter 过滤；不 value-import：host 包携带 scoped-events 的 `Events` merge，进 client program 撞 Context merge）：
