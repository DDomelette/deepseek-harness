...
每个 Host 进程生成随机启动令牌，并由应用根 context 跨 Connection 热重载保留。`dsh-web-app` 每个进程只打印并打开一次 query 中带该令牌的普通根 URL。`frontend-static` 请求 Connection 授权 index 响应：只有 `GET /?token=...` 会把进程令牌交换为 cookie，再重定向到干净的 `/`；API 路径和 Authorization header 都不接受该令牌。过时令牌如果同时带有有效 cookie，会重定向到干净的 `/`。缺失与无效凭据得到同一份最小 401 响应，但 Host/Origin 栅栏接受的非回环 authority 例外：它以 401 提供应用外壳并携带 `__DSH_AUTH_REQUIRED__` 启动事实，因为该客户端无法交换进程令牌，所以它被告知去配对，而不是撞上死路。栅栏拒绝的 authority 只得到那份最小文本，永远拿不到外壳。只有回环 authority 执行该交换：局域网 hostname 因此永远无法把打印出的 URL 变成会话，进程凭据及其换来的 cookie 都留在电脑上，而网络客户端以已配对设备身份被接纳（[手机设备配对](../feature/2026-09-12-phone-device-pairing.zh.md)）。非 index 静态资产保持公开。
