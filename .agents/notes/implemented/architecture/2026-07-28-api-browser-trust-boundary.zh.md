...
- 自定义非 loopback 组合必须信任其服务 authority，否则请求会被拒绝；随后仍像每个 loopback 请求一样满足浏览器认证。随附 CLI 默认绑定所有网卡，并接受 `--host 127.0.0.1` 以仅服务本机（[默认绑定](2026-09-13-lan-bind-by-default.zh.md)）；参见 [LAN Web 服务](2026-09-11-lan-web-serving.zh.md)。`--trusted-host` 只扩展 Host/Origin 栅栏，绝不授予身份。
