...
`apps/web/index.html` carries the manifest link, a matching `theme-color`, the `mobile-web-app-capable` and `apple-mobile-web-app-capable` capability tags, an `apple-mobile-web-app-status-bar-style` of `black-translucent`, and an `apple-touch-icon` link to `/apple-touch-icon.png`. iOS takes the home-screen icon from that link rather than from the manifest's icon list, and honors the capability tags without a secure context.
