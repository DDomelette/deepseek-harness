...
- UI：左栏列出"全部"与用户分组及其实时成员数；`+` 按钮弹出命名对话框（空名或重名禁用"保存"），悬停按钮删除自建分组。选中分组会把右栏过滤为其成员，并显示"添加插件"按钮，其选择器列出未进组的条目，带复选框、搜索框、实时"已选择 N 个"计数以及取消/添加。成员卡片带"移出分组"按钮；视口窄于 680px 时两列堆叠。
