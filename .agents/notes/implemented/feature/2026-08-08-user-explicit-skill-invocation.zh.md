...
- 客户端沿用[纯文本引用决策](../architecture/2026-07-25-web-input-machine-and-slash-pipeline.md)：菜单 pick 落下字面文本 `/name `，该文本随提示词原样提交；ui-skill 不实现任何裁决钩子，也没有引用 codec。调用菜单 RPC `skill.list` 提供每一个用户可调用的 skill 并携带 `modelInvocable`，供菜单标出仅限用户的条目。[用户可控 skill 设置](2026-08-14-skill-settings-panel.md)另行提供的 `skill.catalog` RPC 是配置目录，而不是调用协议。与宿主命令同名的名称解析为命令——客户端会在该行成为提示词之前完成裁决并将其认领。
