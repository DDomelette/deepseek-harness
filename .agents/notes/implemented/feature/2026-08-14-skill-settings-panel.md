...
The Gateway exposes `skills.catalog` addressed by `sessionId`, using the Session controller's preset-scoped resolution. It returns all discovered skills with effective invocation flags, group, source and disabled markers. The generated Remote face and event allowlist forward `skills/change`; settings writes use the existing revision-guarded namespace API.
...
- The `skills.catalog` wire rows expose `source` and `group` strings (the former previously host-side vocabulary): consumers localize the known sources and render unknown ones verbatim.
