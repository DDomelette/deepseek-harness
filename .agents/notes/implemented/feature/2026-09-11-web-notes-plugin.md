...
The Host half carries the whole Remote namespace and the browser half reaches it, so a panel can list conversations and materials, edit a draft, submit it, read the answer back, and ask a follow-up without a rule of its own. Collection ships on both sides of the conversation: the selection bubble and the panel's own screenshot control write materials, and a collected passage carries the identities of the row it came from. What is missing is the way back to that row — no entry point opens a material's source, because the conversation's public face offers no view navigation to a panel outside its slot tree — and the paste path that would collect a screenshot in the conversation rather than in the panel.
