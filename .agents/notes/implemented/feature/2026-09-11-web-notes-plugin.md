...
### A settlement reaches the panel as a forwarded Host event

Settling happens on the Host's clock, so no browser operation is waiting for it: a detail opened while the material was `analyzing` kept showing the material without its answer until the reader refreshed by hand. `Analysis` therefore emits `notes/material-settled` with the conversation and the materials that moved, once per conversation and only when at least one material moved, and the package's `./remote-events` subpath declares that name into the Remote event selection so the app's allowlist forwards it to the page.
...
### The thread draws the model's answer as Markdown

The model answers in Markdown, and the detail drew every thread row as plain text, so a list, a table, and emphasis reached the reader as punctuation. Assistant rows render through `ui-primitives`' `MarkdownText` now; the collected passage and the reader's own follow-up stay as written, because those are the text as it was stored rather than generated output. The primitive's labels — the code block's copy control and its visually hidden footnote heading — come from the panel's own dictionary, the way `ui-trajectory` supplies them for its surface, so no surface inherits another's copy.
...
Those crumbs end at the listed path's own volume, so a Windows browser standing at `C:\` has no parent to step into and could never reach `D:\`. `DirectoryListing` therefore carries an optional `drives` — the mounted volume roots, probed per listing because a volume mounted later must still appear and Windows offers no enumeration without a process — and the browse backend reports it on Windows only. The card's control at a level with no parent opens that volume list, and picking one lists it as any other level; on a single-root platform the field is absent and the control is not drawn, so nothing about the POSIX path changes.

### The model override names a route the deployment serves

The card's model picker reads `session.modelCatalog()` — the same Host-generation catalog the conversation's own model picker renders, which already answers with the deployment's provider groups, each model's display name, and the reasoning efforts that route declares. The session namespace joins the notes Remote namespace and the directory picker as a narrowed face the panel calls, so the notes settings offer what this deployment can actually route to instead of a second, drifting list of what is configured. A stored override the catalog no longer advertises stays selectable: the catalog is advisory, and dropping the reader's stored route would silently rewrite their section.

An override may now name a reasoning effort alongside the route, and `NoteSessions.create` passes it as `agentOptions.reasoningEffort` — a review-approved use of `dsh-llm`'s `ReasoningEffortId` brand, the same constructor the session controller uses when a session selection names an effort. The picker offers that model's declared efforts and no others; choosing the empty entry stores no effort, which leaves the route's own default in charge. A conversation created from the deployment default carries that selection's effort too, so a notes conversation is routed exactly like one created anywhere else.

### A selection feature is picked, named, and prompted

The settings card lists the configured selection features in a picker and edits one at a time: the editor holds a name and a prompt, and the add row below the picker opens an empty editor for a feature the list does not carry yet. Saving writes the complete list, because the list is one document value; the added feature takes the next free `custom-N` id, since a material stores the id it was collected under and the reader may rename the label at any time. The Host still decides — a blank id, label, or prompt, or a duplicated id, is refused as `invalid-actions` — and the card's disabled save is the courtesy, not the enforcement. The prompt field carries the placeholder the reader types over, and the built-in `translate` feature's default prompt is a schema default, so a deployment that never wrote a list picks up its wording without a code change.
...
`packages/notes/notes/tests/` covers both halves at per-file 100% coverage: the domain over a real storage stack, material ordering and archiving, conversation records and the active pointer, conversation creation over the configured workspace and model (including joining a preset roster, registering the directory as a workspace, joining the conversation to that workspace, titling the Session, and disposing the agent when the record fails), thread attribution and projection, body composition, the settings section over a memory provider (including the unset semantics of a write and a refused action list), analysis orchestration against a stand-in agent registry (including the `notes/material-settled` event a closing turn emits and its silence when nothing settled), every Remote operation on its success and refusal path, and the browser half's registrations (both entries over one store instance, which the bubble's collection test asserts, and the settlement revision the panel registration publishes), commands (including the chooser's chosen, cancelled, and refused answers and the browse levels it lists), refusal lines, the DOM anchor reader (including a row that carries no identity), rendered list and detail (including the locate entry, a draft that stays with its material, an answer rendered as Markdown, and the collected body kept as written, and a settlement re-reading the conversations while the panel is mounted), the pasted screenshot, the directory browser descending and stepping back up, and the settings card against a scripted Remote face.
