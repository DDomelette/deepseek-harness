...
`packages/notes/notes/tests/` covers both halves at per-file 100% coverage: the domain over a real storage stack, material ordering and archiving, conversation records and the active pointer, conversation creation over the configured workspace and model (including joining a preset roster and disposing the agent when the record fails), thread attribution and projection, body composition, the settings section over a memory provider (including the unset semantics of a write and a refused action list), analysis orchestration against a stand-in agent registry, every Remote operation on its success and refusal path, and the browser half's registrations, commands, refusal lines, the DOM anchor reader (including a row that carries no identity), rendered list and detail (including the locate entry and a draft that stays with its material), the pasted screenshot, and settings card against a scripted Remote face.
