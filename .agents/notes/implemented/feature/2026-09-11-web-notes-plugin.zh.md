...
目录字段指的是一个目录，而裸输入框只能靠手打路径。宿主自己的选择设施早已以 `directoryPicker` Remote 命名空间的形式挂在 wire 上，因此卡片在字段旁边画一个选择按钮，而不是自己造一个浏览器目录对话框、或去引入 workspace 插件的浏览器半边。`notesFace` 把这个命名空间作为收窄后的一个面接收，并像 notes 命名空间那样以失败作答——是卡片画出来的失败，而不是它要捕获的异常。取消选择什么都不改；被拒绝的选择回报 `directory-unavailable`，与两种图片拒绝一样是面板本地的 code，因为宿主根本看不到这次调用。字段保持可编辑：选择器不提供原生对话框的部署仍要让读者能写出目录。
...
`packages/notes/notes/tests/` 以逐文件 100% 覆盖率覆盖两个半边：真实存储栈上的域、素材排序与归档、会话记录与活动指针、按配置的工作区与模型创建会话（含加入 preset roster、把目录登记为工作区并把该会话加入其中、给 Session 命名，以及记录失败时释放 agent）、线程归属与投影、正文组装、内存 provider 上的设置节（含写入的取消语义与被拒绝的动作列表）、针对替身 agent 注册表的分析编排、每个 Remote 操作的成功与拒绝路径，以及浏览器半边的注册（两个项共用一个 store 实例，由气泡的收集测试断言）、命令（含目录选择器的选中、取消与被拒三种作答）、拒绝文案、DOM 锚点读取器（含不带身份的行）、列表与详情渲染（含定位入口与不跨素材的草稿）、粘贴截图、设置卡（对照脚本化的 Remote 面）。
