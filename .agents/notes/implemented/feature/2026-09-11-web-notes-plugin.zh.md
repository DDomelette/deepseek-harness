...
`materialAnalyze` 拒绝截图（`image-not-submittable`）。截图素材的正文是一条已存下的附件引用，组装它的请求只会得到一条没有指名任何素材的消息——只剩下动作模板，或者干脆是空正文——因此面板解释的规则就是 `Analysis.analyse` 经 `hasComposableBody` 执行的规则。
...
面板在窗格够宽时排成两栏，低于 560px 时只排一栏；这个切换是对面板自身而非窗口的 container query——右列的宽度不是视口的宽度。它的可编辑性跟随宿主的规则而不是靠猜：wire 摘要带上由素材已记录消息 id 派生的 `submitted`，因此正文恰好在素材还是草稿时可编辑，追问输入框则恰好从它不再是草稿时起绘制。导航栏的会话胶囊列出全部会话（含已归档），因为笔记会话从不被删除：选中已归档的那条会把它取回，而两个选择背后的操作早已存在于该命名空间上。
...
选图是面板自己的控件，而不是对会话的截取：浏览器把文件读成规范 base64，因为那正是附件存储在 wire 上接受的形态；格式不受支持或浏览器读不出来都会如实报告，不去打扰宿主。素材留下的是存储返回的 `attachmentId` 而不是那份引用本身，因此读取方无从还原请求所需的 media type、字节长度与尺寸；分析于是回报 `image-not-submittable`，而不是发出一条没有指名任何素材的请求，自动提交的部署也只会把截图存成草稿。把已存引用解析回模型请求这件事仍然空缺。
