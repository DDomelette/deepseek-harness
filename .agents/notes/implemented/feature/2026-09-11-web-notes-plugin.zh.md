...
### 线程把模型的答复按 Markdown 绘制

模型用 Markdown 作答，而详情把每一行都当纯文本绘制，于是列表、表格与强调都以标点的样子到达读者。现在助手行经 `ui-primitives` 的 `MarkdownText` 渲染；收集来的段落与读者自己的追问保持原样，因为那是按原样存储的文本，而不是生成出来的输出。该原语需要的文案——代码块的复制控件与视觉隐藏的脚注标题——取自面板自己的字典，做法与 `ui-trajectory` 为自己的界面提供文案相同，因此没有任何界面继承别人的文案。
