...
只有仍处于 `analyzing` 的素材会被推进，并且这次写在域自己的读-改-写内部再校验一次，因此竞争失败的那次落定会原封不动地留下胜者的结果。完成的 turn 让素材变成 `analyzed`；其它任何结束方式都让它变成 `failed`，并带上事件里的原因——失败自身的 message，或该结束方式的 kind。因此从未闭合的 turn 会让素材停在 `analyzing`，这是诚实的状态：日志里没有任何东西说明它是怎么结束的。
