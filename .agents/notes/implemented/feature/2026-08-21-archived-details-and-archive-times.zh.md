...
已归档设置页此前每个已归档会话只显示一个标题。用户无法知道会话何时被归档，也无法在不恢复的情况下查看其归档前的上下文（工作区分组、cwd、Agent 预设）。按归档前工作区分组早已可用——[归档集合笔记](../../archived/feature/2026-07-31-session-archive-global-set.md)保留了被归档会话的 `sessionIds` 席位——但归档时刻本身没有任何记录。
