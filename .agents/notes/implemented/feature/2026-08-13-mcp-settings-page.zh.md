...
`dsh-mcp-client` 本身不做改动：管理器为每台启用的设置条目热挂载一个实例。
...
`env` 与 `headers` 是 `role('secret')` schema 字段。协议从不回传它们，因此客户端的每次写入都点名它要写的叶子：启用开关写路径 `[serverName, enabled]`；编辑器为每个变更字段发一条逐字段深层路径 op，留空的 env/headers 字段完全不进入 patch。这需要为客户端 settings scope 契约扩展 `SettingsScope.setPath`——此前共享契约只暴露整字段 set/unset，而从脱敏视图重建的整字段写会静默删除已存 secret。

### 状态上报

gateway 只上报挂载生命周期（connecting → ready/failed），不上报存活状态：`failOnStartupError: false` 时不可达的服务器仍会激活，`dsh-mcp-client` 在内部重连，因此 `ready` 表示激活已结算，而非服务器应答成功。标签页的状态点文案相应为「运行中／启动失败」。`dsh-mcp-client` 的公开 API 无任何新增；监督器只包装挂载 fiber。
...
- `packages/mcp/mcp-manager`：schema、注册与写时拒绝、监督器 diff 与挂载/卸载/重挂载、gateway 合并；逐文件 100% 覆盖。
- `packages/client/ui-settings-mcp`：标签页、添加表单、编辑表单、控制器逐叶子写入；100% 覆盖。
- `apps/web/tests/mcp-config.e2e.ts`：真实 scaffold——新增/开关/删除写穿到 `$DSH_HOME/settings.yaml`、搜索、以及 memorix 示例 overlay 下的声明式行；aria golden 位于 `snapshots/mcp-config`。
- `SettingsScope.setPath`：`packages/client/ui-settings/tests/settings-scope.client.spec.ts` 中的单元测试。
